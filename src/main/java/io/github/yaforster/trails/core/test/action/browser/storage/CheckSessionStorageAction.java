...
@SuperBuilder
@Getter
public class CheckSessionStorageAction extends Action {

	private final String sessionStorageItemKey;

	private final ValueComputationInstruction computationInstruction;

	@Override
	public Result execute(TestExecutionContext context) {
		String webStorageItem = context.browser().sessionStorageValue(getSessionStorageItemKey());
		if (StringUtils.isNotBlank(webStorageItem)) {
			return searchForTextInKey(webStorageItem, context);
		}
		else {
			return buildKeyDoesNotExistFailure(context);
		}
	}
	...
	private ValidationFailure buildKeyDoesNotExistFailure(TestExecutionContext context) {
		ValidationFailure.ValidationFailureBuilder<?, ?> builder = ValidationFailure.builder()
			.actionID(getActionID())
			.resultMessage(getKeyDoesNotExistMessage())
	...
	private String getKeyDoesNotExistMessage() {
		return getSessionStorageItemKey() + " does not exist.";
	}

	private String getSuccessMessage(String webStorageItem) {
		return getSessionStorageItemKey() + " does contain the target value. Complete content: " + webStorageItem;
	}

	private String getContentDoesNotExistInKeyMessage() {
		return getSessionStorageItemKey() + " exists but does not contain the target value.";
	}
